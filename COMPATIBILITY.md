...
| --- | --- | --- |
| 1.0.0 release DMG and copied-app launch | Passed | Compressed DMG passes `hdiutil verify` and SHA-256 validation. Read-only mount contains the app, Applications shortcut, and installation instructions. App copied from the image reports 1.0.0 (1), has both architectures, passes strict signature verification, matches all 450 asset hashes, and launches with an isolated roster. |
| Developer ID signing and notarization | Unavailable | No valid code-signing identity is installed. The released app is ad hoc signed and unnotarized; Gatekeeper may block downloaded copies. |
| SwiftPM debug build and sixteen unit tests | Passed | `./script/test.sh`: 16 passed, 0 failed, after adding cats, deer, and more Pixel colors. |
| Updated sprite assets and bundle resources | Passed | All 450 GIFs decode; generated Realistic frames are 128 px and Pixel frames are 32 px, with transparent borders. Pixel GIFs use at most 15 opaque colors. Both bundles match the manifest hashes and include the app icon; all 294 GIFs from before the catalog expansion are unchanged. All eight poses for every new coat were visually inspected. |
| Style migration and persistence | Passed in automated tests | Legacy saves default to Realistic; the Pixel selection persists. Switching styles preserves pet IDs, names, order, hidden state, and original coats. All existing variants resolve to Pixel assets for every action. All 35 Pixel coats, including colors unavailable in Realistic, can be adopted and reloaded without data loss; cats and deer exist in both catalogs. |
| Style switch in native manager and overlay | UI inspection blocked | The expanded-catalog universal app launched with an isolated roster of cats, a deer, and a blue crab; native UI inspection timed out. Shelter interaction/layout and live overlay changes (including paused/asleep pets) remain unverified. Earlier manager/overlay observations below predate this update. |
